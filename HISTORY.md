...
declares `analysis/scripts/_stats.py` as an input, and `paper sync` rewrites
that file's GENERATED header with the new release. 4.1.1 had only exempted
`analysis/scripts/_toolchain/`.

- **A file `paper sync` wrote is hashed without the release in its header.**
  `hashcache.recorded_sha()` and `input_current()` (and so `check-stats`,
  `check-assets` and the generators' `declared_inputs()`) remove the release
  from the ``GENERATED by `paper sync` from paper-scaffold X.`` line of any
  file that starts with that header (after an optional shebang). The rest of
  the header and the whole body still count: an edit to the body reads stale.
  `_provenance.code_inputs()` hashes the modules it records the same way.
  Files without the header are hashed whole, as before.
- **Build records are unchanged.** `paper.pdf`, `paper.docx` and slide decks
  still read stale after a pin move, on purpose: they record `uv.lock` and the
  installed build tools whole, so a new toolchain means a rebuild.

Upgrade: `uv add` the v4.1.2 pin, `uv run paper sync`, then `just paper`.
Nothing in the manuscript changes. A figure or number whose record lists a
synced file (`analysis/scripts/_stats.py`, say) was written in the 4.1.1 form,
a hash of the raw bytes: it is accepted while the file is unchanged, but reads
stale once on this pin move, since the old release cannot be recovered from a
hash. `just assets` re-records it, and later pin moves stay current.
