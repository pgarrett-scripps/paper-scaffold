...
narrate, and check. Replace it. What matters is that the sections below use every
construct the surrounding tooling treats specially, because a construct that
appears nowhere in the skeleton is a construct whose handling silently rots until
the first real manuscript trips over it.

Citations are one such construct. The word counter excludes them, the readability
report drops them, and the narrator deletes them rather than reading a key aloud,
so the skeleton carries two @lovelace1843 @hopper1952 to prove all three still
work. Cross-references are another, both the ordinary kind that resolves to
@sec:methods and the bare-number kind that prints as Section #refn(<sec:methods>)
through the helper defined above.

= Methods <sec:methods>

Inline mathematics is the construct most likely to break, because Typst's HTML
export discards equations outright and the Word path only survives them by way of
the `html.frame` show rule in this file's preamble. A sentence that leans on a
symbol such as $alpha$ or a small expression such as $t_"obs" <= t_"max"$ would
be gutted without it, which is exactly the failure that is easy to miss until a
co-author opens the .docx.
...
Block code and configuration dumps are exempt from the word count and are skipped
by the narrator, so the skeleton includes one:
...
Inline `code` terms, by contrast, are counted as ordinary words, because journals
treat a parameter name in running text as a word like any other.
...
checker compares byte-for-byte against an upstream analysis tree. @fig:example is
here to exercise all three.

#figure(
  image("figures/example_figure.png", width: 70%),
  caption: [
    A placeholder figure. Captions are excluded from the journal word count and
    are not narrated. This image is regenerated by `scripts/gen_example_figure.py`
    and is listed in `figures.map`, so `just check` will report it as stale if the
    upstream copy changes.
  ],
) <fig:example>

Tables behave the same way in the word count, but unlike figures they survive the
Word export as real Word tables rather than as images. @tbl:example is a
hand-written one.
...
#figure(
  ...
  caption: [A hand-written table. Compare with the auto-generated one in the
    Supporting Information, which is written by a script and must never be edited
    by hand.],
) <tbl:example>
