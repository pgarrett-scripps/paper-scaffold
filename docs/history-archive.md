...
Release notes for paper-scaffold 1.0.0 through 3.19.0, moved out of
[HISTORY.md](../HISTORY.md) to keep that file short. Nothing here was edited.
The versioning policy, the upgrade procedure, "Decisions reversed" and
"Bugs worth remembering" stay in HISTORY.md. `just upgrade-plan` reads the
"Upgrade:" lines in both files, so a project on an old release still gets
every step.

This file lives in the scaffold checkout only; new papers do not carry it.
