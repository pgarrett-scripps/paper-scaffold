...
| `just check-submission` | Fail if a file in `submission/` is behind its source (naming the changed files), the cover letter is over the profile's `[cover-letter]` limit, or the availability statement has a gap; then `check-layout` on the upload set. Not in `verify`; in `preflight` |
