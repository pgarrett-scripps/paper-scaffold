...
# ------------------------------------------------ synced file headers ---
# `paper sync` heads every file it writes (analysis/scripts/_stats.py and the
# other helpers, stats.typ, the justfile, ...) with a GENERATED block naming
# the release (src/paper_scaffold/sync.py header_lines). A generator that
# records one as an input -- cascade/paper's gen_stats.py declares _stats.py
# -- then read stale on every pin move though no line of code changed. So for
# a file that carries the header, the recorded hash is of its bytes with the
# release removed from that one line; the rest of the header and the whole
# body still count, so a real change to the file still reads stale (4.1.2).
# A record made before 4.1.2 holds the raw bytes' hash: it is accepted while
# the file is unchanged, and reads stale once after the next pin move, since
# the old release cannot be recovered from a hash (HISTORY.md 4.1.2).
_SYNC_HEADER = re.compile(
    rb"\A((?:#![^\n]*\n)?(?:#|//) GENERATED by `paper sync` from paper-scaffold )"
    rb"[^\s]+?(\. Do not edit:)")
_HEADER_PROBE = 512


def _without_sync_release(data: bytes) -> bytes | None:
    """The bytes with the sync header's release removed, or None if no header."""
    if b"GENERATED by `paper sync`" not in data[:_HEADER_PROBE]:
        return None
    out, n = _SYNC_HEADER.subn(rb"\1\2", data, count=1)
    return out if n else None


def _synced_sha(path: Path) -> str | None:
    """Header-normalized hash of a file `paper sync` wrote, else None."""
    with path.open("rb") as fh:
        if b"GENERATED by `paper sync`" not in fh.read(_HEADER_PROBE):
            return None
    body = _without_sync_release(path.read_bytes())
    return None if body is None else "sha256:" + hashlib.sha256(body).hexdigest()
