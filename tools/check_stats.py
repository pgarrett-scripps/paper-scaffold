...
# What origin.source may be: a repo path, a URL, a DOI, a commit, or an
# evidence set. A note says why the number is right; the source says where a
# reader re-reads it (5.0.0, docs/numbers.md).
SOURCE_FORMS = re.compile(
    r"^(?:https?://\S+|doi:\S+|10\.\d{4,}/\S+|evidence:[\w.-]+(?:/\S*)?"
    r"|(?:[\w.-]+@)?[0-9a-f]{7,40}|[\w.-]+(?:/[\w.@+-]+)*(?:#\S+)?)$")


def _hand_source(id: str, o: dict) -> list[Finding]:
    """A hand entry names a source a reader can go back to (5.0.0).

    An error for a new entry. An entry `paper sync` found without one when the
    paper moved to 5.0.0 is listed in .paper/grandfathered.json and warns
    instead, so the upgrade itself fails nothing.
    """
    import evidence
    src = o.get("source")
    if not isinstance(src, str) or not src.strip():
        level = ("warn" if id in evidence.grandfathered(ROOT, "hand-source")
                 else "error")
        return [Finding(level, id,
            "is hand-entered with a note but no origin.source: add the path, "
            "URL, DOI, commit or evidence:SET a reader can re-read it from")]
    src = src.strip()
    if evidence.HOST_PATH.match(src) or src.startswith("/"):
        return [Finding("error", id,
            f"origin.source {src} is a host path; give it relative to the "
            f"manuscript root, or as evidence:SET")]
    if not SOURCE_FORMS.match(src):
        return [Finding("error", id,
            f"origin.source {src!r} is not a path, URL, DOI, commit or "
            f"evidence:SET")]
    if src.startswith("evidence:"):
        try:
            manifest = evidence.load(ROOT)
        except ValueError as exc:
            return [Finding("error", id, str(exc))]
        name = src[len("evidence:"):].split("/", 1)[0]
        if manifest is None or name not in manifest.sets:
            return [Finding("error", id,
                f"origin.source names evidence set {name!r}, which "
                f"evidence.toml does not declare")]
    elif (not re.match(r"https?://|doi:|10\.\d{4,}/", src)
          and not re.fullmatch(r"(?:[\w.-]+@)?[0-9a-f]{7,40}", src)):
        if not (ROOT / src.split("#", 1)[0]).exists():
            return [Finding("warn", id,
                f"origin.source {src} is not present in this checkout")]
    return []
