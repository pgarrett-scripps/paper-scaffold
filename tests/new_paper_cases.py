...
def run_cases() -> bool:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        checks = [
            ...
            # The toolchain is the pinned package (docs/package.md), not a
            # copy: tools/, tests/, docs/, journals/ and word/ stay behind, and
            # `paper sync` writes the files a paper must hold, with its lock.
            ("no tools/, tests/, docs/, journals/, word/",
             not any((dest / d).exists()
                     for d in ("tools", "tests", "docs", "journals", "word"))),
            ("no package source or HISTORY.md",
             not (dest / "src").exists() and not (dest / "HISTORY.md").exists()),
            ("justfile generated by paper sync",
             "GENERATED by `paper sync`" in (dest / "justfile").read_text()
             if (dest / "justfile").is_file() else False),
            ("analysis helpers generated",
             (dest / "analysis" / "scripts" / "_toolchain" / "paths.py").is_file()
             and "GENERATED" in (dest / "analysis" / "scripts" / "_stats.py").read_text()),
            ("lock written", (dest / ".paper" / "scaffold.lock.json").is_file()),
            ("docs mirrored for reading", (dest / ".paper" / "docs" / "README.md").is_file()),
            ("docs mirror ignored", ".paper/docs/" in (dest / ".gitignore").read_text()),
            ...
            # The toolchain's licence travels with the package; a LICENSE at
            # the root of a manuscript would read as the paper's.
            ("no LICENSE", not (dest / "LICENSE").exists()
             and not (dest / "LICENSE.scaffold").exists()),
        ]
        ...
        # The pin, this release's tag by default, and no toolchain package
        # metadata (the paper is not the package).
        checks.append(("pyproject pins this release",
                       f"paper-scaffold@v{version}" in pyproject
                       and "[project.scripts]" not in pyproject
                       and "force-include" not in pyproject))
